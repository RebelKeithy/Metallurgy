...
public class EntityOreFX extends EntityFX
{
    ...
    public EntityOreFX(World par1World, double par2, double par4, double par6, float par8, float par10, float par12, int id)
    {
        ...
        this.noClip = true;
        this.setParticleTextureIndex(Math.random() > 0.5f ? 2 + (16*id) : (16*id));
        decending = getParticleTextureIndex()%16 > 0;
        this.id = id;
        rotation = (int) (Math.random()%90);
    }
	...
	public void renderParticle(Tessellator tessellator, float f, float f1, float f2, float f3, float f4, float f5)
	{
	    ...
	    GL11.glBindTexture(3553 /*GL_TEXTURE_2D*/, ModLoader.getMinecraftInstance().renderEngine.getTexture("/shadow/EnderParticles.png"));
    ...
    public void onUpdate()
    {
        ...
        {
        	setParticleTextureIndex(1+(16*id));
        }
            if(particleAge == particleMaxAge/3 + particleMaxAge/3 )
        {
        	if(decending)
        		setParticleTextureIndex(16*id);
        	else
            	setParticleTextureIndex(2+(16*id));
        }
